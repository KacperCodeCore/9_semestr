...
context("automationteststore", () => {
  ...
  it("1. Odwiedzenie strony i sprawdzenie tytułu Odwiedź stronę główną aplikacji i sprawdź, czy tytuł strony jest poprawny.", () => {
    cy.title().should("equal", "A place to practice your automation skills!");
  });
  ...
  it("4. Wybór opcji z rozwijanego menu Odwiedź stronę z formularzem, wybierz opcję z rozwijanego menu i wyślij formularz. ", () => {
    cy.get("#categorymenu").contains("Apparel & accessories").realHover();
    cy.get("#categorymenu")
  ...
  it("5. Sprawdzenie obecności elementu po kliknięciu Odwiedź stronę, kliknij przycisk i sprawdź, czy nowy element pojawia się na stronie.", () => {
    ...
    cy.get(".cart").click();
    cy.reload();
    cy.get(".product-list > .table > tbody > tr").should(
  ...
  it("6. Nawigacja do strony i sprawdzenie obecności tekstu Odwiedź stronę i sprawdź, czy określony tekst jest obecny. ", () => {
    cy.get("span.maintext").should("include.text", "Scrolling");
  });
  ...
  it("8. Wypełnienie formularza z walidacją Odwiedź stronę z formularzem, wypełnij go niepoprawnie, sprawdź komunikaty walidacyjne, a następnie popraw i wyślij. ", () => {
    ...
    cy.get("#ContactUsFrm_first_name").type("Alicja");
    cy.get("#ContactUsFrm_email").type("sdasdasdasd");
    cy.get("#ContactUsFrm_enquiry").type("text");

    cy.get(
      'button[type="submit"].btn.btn-primary.lock-on-click[title="Submit"]'
    ).click();

    cy.get("div.element_error.has-error").then(($error) => {
      if ($error.is(":visible")) {
        cy.get("#ContactUsFrm_email").type("ContactUsFrm_email@gmai.com");
        cy.get(
          'button[type="submit"].btn.btn-primary.lock-on-click[title="Submit"]'
        ).click();
      }
  ...
  it.only("9. Interakcja z elementem dynamicznym Odwiedź stronę, kliknij przycisk, aby załadować dynamiczny element, a następnie sprawdź jego zawartość. ", () => {
    cy.get("#block_frame_featured_1769 > .thumbnails > :nth-child(1)").click();
    cy.get("#maincontainer > .container-fluid").should("be.visible");
  });
  ...
  it("10.  Sprawdzenie przekierowania po zalogowaniu Odwiedź stronę logowania, zaloguj się i sprawdź, czy nastąpiło przekierowanie do strony głównej. ", () => {
    ...
    cy.get("#loginFrm_loginname").type("kkkkkksdfsfsedc");
    cy.get("#loginFrm_password").type("%#TGSDfsdzbsfd");
